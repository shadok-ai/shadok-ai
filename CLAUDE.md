...
| `src/peers.ts` | **Cross-instance peers** — the account model for MACHINES: another instance/harness allowed to reach THIS one's agents (`~/.shadok-ai/peers/<key>.json`, 600, per launch dir). A peer's credential is a token shaped exactly like the agent session key (invariant 33): `signPeerToken(name)` = HMAC of the name, **non-expiring** (a cross-instance link is persistent and must not hit the 7-day cookie cliff) and **revocable by the registry** — `peerFromToken` verifies the MAC AND that the name is still listed, so deleting the row kills every token for it with no server state. Presented as `x-shadok-peer`. A peer is a DISTINCT principal (`peerName` in server.ts, alongside `currentAccount`), NOT a member: over HTTP it reaches ONLY `GET /diff` of its own agents; its spawn/drive path is the WS. `ownedByPeer(channel, name)` is the one ownership rule behind every scope (resume/stop/diff) — a peer only ever touches agents whose `Channel.createdByPeer` is its own (tagged at `start`, invariant 24). `DELETE /peers` revokes: future auth fails AND the peer's live WS connections are dropped. Admin-only management (`/peers`, browser-only like `/users`). Pure cores tested (`test/peers.test.ts`); the boundary is proven end-to-end (a peer spawns+is-tagged, cannot resume/stop/diff a local agent, is denied `/secrets`/`/peers`, and revocation cuts HTTP+WS). Design + follow-ups (the outbound `pilotctl --peer`, an open HTTP `/inbox` for non-shadok harnesses, finer scoping): `docs/superpowers/specs/2026-09-26-cross-instance-agent-collaboration-design.md`. |
