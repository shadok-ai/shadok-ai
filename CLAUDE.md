...
| `src/extract.ts` | Parse the transcript / screen: `loadHistory`, `detectDialog`, `listSessions`, `findSessionId`. `readHistoryTurns` is `loadHistory` WITHOUT the `slice(-100)` — the whole history, same filters — so the search engine matches exactly the text a human would have read; `loadHistory` = `readHistoryTurns(…).slice(-100)`. |
| `src/search.ts` | The cockpit's search engine, PURE half (no fs): `normalizeQuery`, `makeSnippet` (a one-line, ellipsised context window around the first case-insensitive match, keeping the text's own casing), `searchTurns` (newest hit first, capped per agent). Substring match on purpose — what "find where I talked about X" means, no regex surprises. `server.ts` reads the transcripts (`readHistoryTurns` over `loadChannels`) and the client renders the hits; tested directly like the front-end pure cores. |
...
`/sessions` `/recover` (resumable), `/search` (GET `?q=` — full-text across this
cockpit's agents; scans each channel's whole transcript via `readHistoryTurns`
and the pure `src/search.ts`, hits newest-first, capped; behind the gate),
`/diff`, `/channels` `/groups` (GET/PUT,
